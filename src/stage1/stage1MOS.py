...
def ERLDM(p,g,type='abs', factor_=0.1): 
    ...
    total = (p_old.shape[0]*len(p)) - len(p)
...
if __name__ == '__main__':
    ...
    trainlist = f'{ref}/table1_data/stage1MOS/train_test/DATA/sets/train_mos_list.txt'
    validlist = f'{ref}/table1_data/stage1MOS/train_test/DATA/sets/val_mos_list.txt'
    ...
    trainloader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=4, collate_fn=trainset.collate_fn)
    validset = MyDataset(wavdir, validlist)
    validloader = DataLoader(validset, batch_size=2, shuffle=False, num_workers=4, collate_fn=validset.collate_fn)
    ...
    for epoch in range(0,1001):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            loss = RLDM(outputs,labels,type=type_,extend=True, batch_size=batch_size, extend_num=extend_num, factor=factor) + mix*criterion(outputs, labels)
